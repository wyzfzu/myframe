...
public class BaseSqlProvider {
    ...
    public Class<?> getEntityClass(MappedStatement ms) {
        ...
        if (entityClassMap.containsKey(msId)) {
            ...
            Class<?> theMapperClass = getMapperClass(msId);
            if (theMapperClass == null) {
                throw new RuntimeException("无法获取Mapper<T>泛型类型:" + msId);
            }
            Type[] types = theMapperClass.getGenericInterfaces();
            for (Type type : types) {
    ...
    public boolean supportMethod(String msId) {
        Class<?> theMapperClass = getMapperClass(msId);
        if (theMapperClass != null && this.mapperClass.isAssignableFrom(mapperClass)) {
